function [x_facil,xff,xsf,x_chungo,xfc,xsc] = data_delicado_heteroscedastic;
...
ro = 1;
sigma_1 = factor*0.25;
sigma_2 = factor*0.001;  % Noise in the 3rd principal direction
A = factor*0.15; 
...
%zz = A*sin((2*n-1)*6*pi/6) + sigma_2*(1 + fact_std*(theta_n)).*randn(1,N);
zz = A*sin((2*n-1)*6*pi/6) + sigma_2*randn(1,N);

x = [xx;yy;zz];

% % med = mean(x')';
% % xx = x - repmat(med,1,length(x(1,:)));
% %
% % [B,L]=eigs(xx*xx');
% 
% [B,L]=eigs(x*x');
...
% 
% yy = y;
% yy(3,:) = yy(3,:) + sigma_2*(1 + fact_std*(y(1,:))).*randn(1,N);
% 
% xx = B*yy;
