...
function [datT2 model] = DRR_normaliz(dat,flag_entropy,flag_cross,flag_trans)
...
for n_dim=1:size(dat2,1)-1
    ...
    model(n_dim).models_coef = models;
    model(n_dim).ii = ii;
    ...
    %%%%%%%%% hallar desviacion de los residuos
    
    residuos = dat2(ii,:)-Ypred_krr';
    abs_residuos = abs(residuos);
    
    %%%%%%%%% entrenar y predecir desviaciones
    if flag_cross == 1
        Y1 = abs_residuos(:,ii_2(1:NN_t));
        Y2 = abs_residuos(:,ii_2(NN_t+1:end));
    else
        Y1 = abs_residuos;
        Y2 = abs_residuos;
    end
    ...
    sigma_pred_krr = test_fast_KRR_DRR(dat2(ii2,:)',models);
    
    %%%%%%%%% almacenar modelo para las desviaciones

    model(n_dim).models_sigma = models;

    %%%%%%%%% normalizar los residuos por las desviaciones

    datT2(end-n_dim+1,:) = (dat2(ii,:)-Ypred_krr')./sigma_pred_krr';        
