...
function datT2 = apply_DRR_normaliz(dat,model)
...
for n_dim=1:size(dat2,1)-1
    ...
    [Ypred_krr] = test_fast_KRR_DRR(dat2(ii2,:)',model(n_dim).models_coef);
    [sigma_pred_krr] = test_fast_KRR_DRR(dat2(ii2,:)',model(n_dim).models_sigma);
